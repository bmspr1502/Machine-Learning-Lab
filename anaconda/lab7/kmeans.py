...
class kmeans:
    ...
    def kmeanstrain(self, data, maxIterations=10):
        ...
        while np.sum(np.sum(oldCentres - self.centres)) != 0 and count < maxIterations:
            ...
            # Compute distances
            distances = np.ones((1, self.nData)) * np.sum(
                (data - self.centres[0, :]) ** 2, axis=1
            )
            for j in range(self.k - 1):
                distances = np.append(
                    distances,
                    np.ones((1, self.nData))
                    * np.sum((data - self.centres[j + 1, :]) ** 2, axis=1),
                    axis=0,
                )
    ...
    def kmeansfwd(self, data):
        ...
        # Compute distances
        distances = np.ones((1, nData)) * np.sum(
            (data - self.centres[0, :]) ** 2, axis=1
        )
        for j in range(self.k - 1):
            distances = np.append(
